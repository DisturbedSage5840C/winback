...
[`docs/EVALUATION.md`](EVALUATION.md) §04–§06, generated from Postgres, and
...
|---|---|---|---|---|
| **1** | 26–27 Aug | Repo, venv, pinned deps, Postgres + schema + append-only DDL, test keys, MCP handshake, live-lane spike | ✅ | [`docs/LIVE_LANE_FINDINGS.md`](LIVE_LANE_FINDINGS.md) — 11 probes, all resolved, no unknowns carried |
| **2** | 28 Aug | Six compliance modules TDD + root-cause lookup | ✅ | [`docs/COMPLIANCE.md`](COMPLIANCE.md); every rule has a test proving it **blocks**; 99% coverage |
| **3** | 29 Aug | Oracle + legacy policy + generator, dataset frozen, realism chart | ✅ | [`docs/DATA.md`](DATA.md), [`docs/assets/realism.png`](assets/realism.png) |
| **4** | 30 Aug | Features + XGBoost + 3-way calibration, model v1 frozen, observed-vs-censored split | ✅ | [`docs/EVALUATION.md`](EVALUATION.md) §05–§07, `ml/artifacts/metrics_v1.json`, [`docs/assets/calibration.png`](assets/calibration.png) |
| **5** | 31 Aug | Policy layer + four-arm paired harness + bootstrap CIs → `EVALUATION.md` | ✅ | [`docs/EVALUATION.md`](EVALUATION.md) §04–§07, [`docs/assets/four_arms.png`](assets/four_arms.png), `eval_runs` / `eval_arm_results` / `eval_arm_violations` / `eval_intervals` in Postgres |
...
| §11 | ~4-minute video | Script firm (`docs/DEMO_SCRIPT.md`) — a plain, conversational four-section walkthrough (intro, repo, website, close), every number pulled from the running system. Simplified from an earlier shot-by-shot draft; recording is yours (§05) |
...
demonstrates it, and it is now **complete**: **190 invoices concluded, 190 audit rows,
₹3,57,468 recovered, zero violations** — 86 recovered, 65 deferred, 22 blocked, 17
written off. One row per conclusion, no exceptions. It resumed through three session-limit
halts to get there, and the "Open" line saying otherwise was stale for a day; queried and
corrected on 4 Sep.
`decisions` holds **191** rows for those 190 invoices, and the extra one is the resume
logic working rather than failing: `inv_1957_01` was approved at 15:14 and killed before
the tool call, leaving a decision with no audit row — the one state that *must* be
re-worked, because skipping it files an approval with nothing behind it. It was re-worked
at 18:59 into a single presentment and a single audit row. The reverse mistake, re-working
an invoice that had concluded, would present a mandate twice, and NPCI counts
presentments, not batches.
...
**`docs/WHAT_BROKE.md` is 1,516 lines written as it happened**, including the two Day-4
...
- **Resolved: `docs/index.html` deleted.** It sat untracked for days — 287 lines of a
  generic dark gradient on a system font stack, none of the Satoshi / `#0e0b08` /
  `#305eff` language the plan settled on — and was never linked from README or served
  by anything (no GitHub Pages config anywhere in the repo). Left on disk it was a
  landing page a judge could stumble into that contradicted the shipped dashboard's
  design; removed rather than committed or replaced, since nothing needs it.
...
| # | Date | What changed |
|---|---|---|
| 15 | 4 Sep 2026 | **Frontend improvements from the Figma Make export's own plan, implemented against the live dashboard — and a formatter defect found and worked around along the way.** A new Figma Make export (`Untitled.zip`) arrived with an embedded planning document that, read first, explicitly argued against a redesign — the live `dashboard/` was judged a deliberate, restrained design system already — and prescribed four small, additive changes instead. Read that as the spec rather than as raw source to swap in wholesale: the zip's own `src/` tree still carried the scaffold clutter (`.gitattributes`, `pnpm-lock.yaml`, eight `.figma/make/` wrapper scripts) untracked from the repo two entries ago, and a `src/data/demo.ts` mock-data file that would have reintroduced the exact violation "no mocks anywhere" exists to prevent. Three of the four items shipped: a `Skeleton`/`PanelSkeleton` pair in `ui.tsx`, shaped to each page's real layout rather than generic, replacing `Spinner` on Overview/Worklist/Compliance/Evaluation/Invoice (`LiveTrace` correctly keeps `Spinner` — its own inline polling state already does that job); route-transition motion in `App.tsx`'s `Shell` via `framer-motion`, gated behind the existing `usePrefersReducedMotion` hook; and a `CopyableId` component wired into `WorklistTable`, `Worklist`'s quick-look drawer, and `Invoice`'s header id and audit-trail chips. The fourth item — a feature-importances panel — was conditional on a backend `/model/importances` endpoint that a grep confirmed did not exist yet, so it was correctly skipped rather than built against nothing (the endpoint was added later the same day — see [`IMPROVEMENTS_LOG.md`](IMPROVEMENTS_LOG.md) — so a reader checking `api/main.py` today will find it, and that is not a contradiction of this entry). Along the way, `npm run format` (`oxfmt`, listed under `AGENTS.md`'s "Checks") reformatted the entire `src/` tree — not just the edited files — and dropped semicolons inside inline TypeScript object type annotations, breaking six files including three never touched by hand; `tsc --noEmit` caught it immediately. All eight edited files were reverted to `HEAD` and redone without the format step, this time verified with `tsc --noEmit` and `npm run build` alone (the plan's own bar, and what `AGENTS.md` actually gates on); a stray `oxfmt` quote-style rewrite of `vite.config.ts` from an earlier pass was found on a whole-tree `git status` sweep and reverted too. `WHAT_BROKE.md` entry 45. Full suite re-verified after: 612 tests pass, `ruff` clean, `tsc --noEmit` and `vite build` clean. Doc stats: 4,570 lines across 11 files, `WHAT_BROKE.md` at 1,616. |
| 14 | 4 Sep 2026 | **Root listing shrunk without touching anything that runs.** Asked whether the GitHub root still looked too big after entry 13's scaffold cleanup — it did: 11 real folders plus 10 loose files is 21 rows, none of it residue this time, all of it legible architecture. Given the choice between nesting the seven Python packages under one parent (touches every import, `pyproject.toml`'s `pythonpath`/`testpaths`, `docker-compose.yml`, both shell scripts — real risk one day before a public gate) and a safe pass, the safe pass won: `COMPLETION_REPORT.md` moved into `docs/` (`git mv`, its own seven relative links and the two inbound references in `README.md` and `WHAT_BROKE.md` fixed to match — nothing else in the tree names its path), and an eleven-line, one-row-per-folder map added under the title in `README.md` so a judge reads the size as organization before they ever open the file browser. Root is 18 rows now. Nothing moved that anything imports, builds, or runs — `npm run build` and the full suite re-verified after. Doc stats: 4,516 lines across 11 files. |
| 13 | 4 Sep 2026 | **A full re-read pass, one stale line found, and the tree cleaned of scaffold GitHub was never meant to carry.** Re-checked the implementation and every doc against the repo rather than against memory: `pytest` (612 tests) and `ruff` both clean, no `TODO`/`FIXME` anywhere in tracked source, no other stale figures except one — this file's own §03 still said `WHAT_BROKE.md` was "1,238 lines," disagreeing with §01's correct 1,516 two paragraphs up; fixed on sight, per this file's own rule. Then the cleanup: `dashboard/.figma/make/` held nine files behind one blanket defense — "`vite.config.ts` imports `site.json` from it, so the directory stays" — true of one file, unread for the other eight (Figma's own hosting-CLI wrappers: `deploy`, `analyze-routes`, `langserver`, and five more). `dashboard/.gitattributes` (Git-LFS rules for ~80 extensions, no LFS anywhere in this repo) and `dashboard/pnpm-lock.yaml` (real, but `npm install` is the tested path and its one reader, `dev.json`, was leaving anyway) were the same shape. All ten `git rm --cached` — untracked, not deleted; every file still sits on disk, and `.figma/make/site.json` is untouched. `AGENTS.md`, `README.md` §06, and the Day-9 `WHAT_BROKE.md` entry that had defended the whole directory were all corrected to name `site.json` specifically. A new `WHAT_BROKE.md` entry (44th) records why the per-file question sat open nine days after the per-directory one was answered. Doc stats: 3,921 lines across 10 files, `WHAT_BROKE.md` at 1,563. |
| 12 | 4 Sep 2026 | **Submission pack written, and writing it caught two stale numbers.** `docs/SUBMISSION.md` is the form answered a day early: the one-line and one-paragraph descriptions, Track 03's wording mapped clause by clause to the evidence, every figure a box might ask for, the limits paragraph said first rather than when asked, and a pre-submit checklist whose first item is opening the video link **logged out** — a Drive link defaulting to "restricted" is the classic way to hand a judge a permission wall. The rule was that no number goes on a form unverified, so each was re-derived from its source rather than copied: the arm table and ECE against `EVALUATION.md`, the batch figures against Postgres. **Two claims did not survive that.** `WHAT_BROKE.md`'s `## Open` section and this report's §03 both still said `batch_v2` was *"75/190 and resuming"* — a day stale; it finished at **190/190, 190 audit rows, ₹3,57,468, zero violations** (86 recovered · 65 deferred · 22 blocked · 17 failed). Both corrected, and the Open item closed rather than quietly deleted, because a report that overstates what is *unfinished* is wrong in the same way as one that overstates what is done — it just fails in the flattering direction, which is why it survived a day. The same query turned up **191 `decisions` rows for 190 invoices**, which is the resume logic working and not failing: `inv_1957_01` was approved at 15:14 and killed *between the guardrail's answer and the tool call*, the one state `_already_worked` must re-work rather than skip, and re-working it produced one presentment and one audit row. Written down in both places before a judge finds the mismatch themselves. `docs/SUBMISSION.md` listed in README §09. |
