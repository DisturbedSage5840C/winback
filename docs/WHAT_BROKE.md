...
## 2026-09-09 · One feature carried a category name that agreed with nothing else in the repo

**Believed.** `ml/features.py`'s `HIGH_CEILING_MCC` — the frozenset that sets
`mcc_is_high_afa_ceiling`, the feature that tells the model an invoice qualifies for
RBI's ₹1,00,000 AFA ceiling rather than the ₹15,000 default — named the same three
merchant categories the compliance rule and the simulator use. It had been written once,
by hand, alongside them, and nothing since had reason to doubt it agreed.

**Actually true.** It did not. `compliance/afa_threshold.py`'s `ELEVATED_MCC_CATEGORIES`
and `sim/generate.py`'s `MCC_MIX` both spell one of the three categories
`"credit_card_bill"`; `ml/features.py` spelled it `"credit_card"`. A frozenset lookup on
a category string that never matches returns nothing wrong — it just returns `False` —
so `mcc_is_high_afa_ceiling` was `0.0` for every invoice in that category: roughly 6% of
the population, and its highest-value slice (₹2,000–₹60,000), the one place the ₹15,000
vs ₹1,00,000 ceiling actually changes which invoices a merchant would historically have
pursued. Nothing caught it because nothing checked it: the frozen-model golden fixture
(`ml/tests/testdata/frozen_model_golden.json`) stores eight pre-built feature vectors
from a seeded RNG, not real category strings run through `features_for`, so it re-derives
scores from the constant but never the constant itself. No test asserted the three
category sets agreed with each other or with what the simulator actually generates.

**Cost.** One feature undervalued for the highest-ceiling slice of the population for as
long as the model existed — measured, not assumed: gain on `mcc_is_high_afa_ceiling`
moved from 0.00477 to 0.00448 after the fix, third-decimal-place money on a model whose
top feature (`prior_root_cause_bd_hard`) carries 45.5%. Retraining on the corrected
feature also moved the headline: Winback's legally-recovered total went from ₹6,39,626 to
₹6,40,525, best iteration from 339 of 600 to 400, and every number in `EVALUATION.md`
§09–§10 that was hand-transcribed from `metrics_v1.json` rather than generated by
`eval.report` had to be re-transcribed by hand, which is its own finding: only §04–§06 of
that document regenerate themselves, and everything past it drifts silently on a retrain
unless someone remembers to chase it down. This entry is that chase, done once, in full,
with every old and new number checked against the artifact rather than against memory.

While tracing it, the same duplication pattern turned up in the NPCI attempt cap:
`compliance/npci_retry_cap.py`'s `MAX_ATTEMPTS_PER_INVOICE = 4` was restated as a bare
literal `4` in `ml/features.py`, `eval/arms.py`, `sim/legacy_policy.py`, and
`agent/tools.py` — four more places a future OC-215-A revision would have to be applied
by hand and could easily miss one.

**Changed.** `HIGH_CEILING_MCC` is now `compliance.afa_threshold.ELEVATED_MCC_CATEGORIES`
imported directly rather than restated, so it cannot drift from the rule module again by
construction — the new
`test_the_high_ceiling_categories_agree_with_the_simulator_and_the_rule` in
`ml/tests/test_features.py` additionally asserts every elevated-ceiling category is one
the simulator actually generates. The four bare `4`s import
`MAX_ATTEMPTS_PER_INVOICE` instead. `python -m ml` retrained the model on the corrected
feature; the frozen-model golden fixture, `docs/EVALUATION.md` (regenerated block via
`eval.report`, hand-written §07/§09/§10 re-transcribed by hand), `sim/legacy_policy.py`'s
module docstring, `eval/counterfactual.py`'s comment, `docs/DATA.md`, `docs/SUBMISSION.md`,
`docs/FRONTEND_SPEC.md`, and `docs/COMPLETION_REPORT.md`'s body all follow the new
figures. While re-transcribing the rupee-confusion table in `EVALUATION.md` §09, a
pre-existing and unrelated units bug surfaced — margin recovered and net were both off by
100× (dividing paise by 10,000 instead of 100: ₹19,752 and ₹19,713 where the same
`metrics_v1.json`, old and new, computes ₹19,75,245 and ₹19,71,345) — fixed in the same
pass since it was already open on the screen.
